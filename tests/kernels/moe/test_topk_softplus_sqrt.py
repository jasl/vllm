...
@pytest.mark.parametrize(
    "num_experts,dtype", [(256, torch.float32), (128, torch.bfloat16)]
)
def test_hash_bounds_guard_preserves_vision_and_special_token_routing(
    num_experts, dtype
):
    torch.manual_seed(7)
    topk = 6
    vocab_size = 64
    input_ids = torch.tensor([-1, 64, 2**31 - 1, 56, 61, 0], device="cuda")
    logits = torch.randn(6, num_experts, device="cuda", dtype=dtype)
    table = torch.stack([torch.randperm(num_experts)[:topk] for _ in range(vocab_size)])
    table = table.to(device="cuda", dtype=torch.long)
    bias_vl = torch.randn(num_experts, device="cuda")
    weights = torch.empty(6, topk, device="cuda")
    ids = torch.empty(6, topk, device="cuda", dtype=torch.int32)
    ...
    _assert_topk_matches(weights[3:], ids[3:], ref_weights, ref_ids)
