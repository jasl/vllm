...
def test_deepseek_v4_mega_moe_finalize_transforms_and_releases_weights(monkeypatch):
    vllm_config = SimpleNamespace(
        compilation_config=CompilationConfig(),
        scheduler_config=SimpleNamespace(max_num_batched_tokens=4),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            transform_sf_into_required_layout=fake_transform_sf_into_required_layout,
            transform_weights_for_mega_moe=fake_transform_weights_for_mega_moe,
        ),
    )
